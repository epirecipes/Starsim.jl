...
Extension providing AMD GPU support for Starsim.jl via AMDGPU.jl.
...
struct GPUPeopleArrays
    alive::ROCVector{UInt8}
    age::ROCVector{Float32}
    female::ROCVector{UInt8}
    slot::ROCVector{Int32}    # CRN slot indices (1-based)
    n::Int
end
...
struct GPUDiseaseArrays
    susceptible::ROCVector{UInt8}
    infected::ROCVector{UInt8}
    recovered::Union{ROCVector{UInt8}, Nothing}
    ti_infected::ROCVector{Float32}
    ti_recovered::Union{ROCVector{Float32}, Nothing}
    rel_sus::ROCVector{Float32}
    rel_trans::ROCVector{Float32}
    n::Int
    # SEIR-specific (nothing for SIR/SIS)
    exposed::Union{ROCVector{UInt8}, Nothing}
    ti_exposed::Union{ROCVector{Float32}, Nothing}
end
...
each timestep via `copyto!` to avoid per-step ROCVector allocation.
"""
mutable struct GPUSim
    ...
    edge_p1::ROCVector{Int32}
    edge_p2::ROCVector{Int32}
    edge_beta::ROCVector{Float32}
    rng_buf::ROCVector{Float32}
    new_infected::ROCVector{UInt8}
    jitter_buf::ROCVector{Float32}
    snap_infected::ROCVector{UInt8}      # Snapshot buffer for synchronous transmission
    snap_susceptible::ROCVector{UInt8}   # Snapshot buffer for synchronous transmission
    edge_capacity::Int
    ...
    rng_seeds::ROCVector{UInt32}
    # CRN support: base seeds (per-agent), reset each timestep
    crn_mode::Bool
    rng_seeds_base::ROCVector{UInt32}   # per-agent base seeds (deterministic from sim seed)
    rng_seeds_agent::ROCVector{UInt32}  # per-agent current seeds (reset each timestep)
end
...
function _people_to_gpu(people::Starsim.People)
    ...
    GPUPeopleArrays(
        ROCVector{UInt8}(UInt8.(people.alive.raw)),
        ROCVector{Float32}(Float32.(people.age.raw)),
        ROCVector{UInt8}(UInt8.(people.female.raw)),
        ROCVector{Int32}(Int32.(people.slot.raw)),
        n,
    )
end
...
function _sir_to_gpu(d::Starsim.SIR)
    ...
    GPUDiseaseArrays(
        ROCVector{UInt8}(UInt8.(d.infection.susceptible.raw)),
        ROCVector{UInt8}(UInt8.(d.infection.infected.raw)),
        ROCVector{UInt8}(UInt8.(d.recovered.raw)),
        ROCVector{Float32}(Float32.(d.infection.ti_infected.raw)),
        ROCVector{Float32}(Float32.(d.ti_recovered.raw)),
        ROCVector{Float32}(Float32.(d.infection.rel_sus.raw)),
        ROCVector{Float32}(Float32.(d.infection.rel_trans.raw)),
        n,
...
function _sis_to_gpu(d::Starsim.SIS)
    ...
    GPUDiseaseArrays(
        ...
        ROCVector{Float32}(Float32.(d.infection.ti_infected.raw)),
        ROCVector{Float32}(Float32.(d.ti_recovered.raw)),  # SIS uses ti_recovered for timing
        ROCVector{Float32}(Float32.(d.infection.rel_sus.raw)),
        ROCVector{Float32}(Float32.(d.infection.rel_trans.raw)),
        n,
...
function _seir_to_gpu(d::Starsim.SEIR)
    ...
    GPUDiseaseArrays(
        ...
        ROCVector{UInt8}(UInt8.(d.exposed.raw)),
        ROCVector{Float32}(Float32.(d.ti_exposed.raw)),
    )
end
...
function Starsim.to_gpu(sim::Starsim.Sim)
    ...
    edge_p1 = ROCVector{Int32}(zeros(Int32, edge_cap))
    edge_p2 = ROCVector{Int32}(zeros(Int32, edge_cap))
    edge_beta = ROCVector{Float32}(zeros(Float32, edge_cap))
    rng_buf = ROCVector{Float32}(zeros(Float32, edge_cap))
    new_infected = AMDGPU.zeros(UInt8, n)
    jitter_buf = ROCVector{Float32}(zeros(Float32, n))
    snap_infected = AMDGPU.zeros(UInt8, n)
    snap_susceptible = AMDGPU.zeros(UInt8, n)
    ...
    rng_seeds = ROCVector{UInt32}(edge_seeds)
    ...
    rng_seeds_base = ROCVector{UInt32}(agent_base_seeds)
    rng_seeds_agent = ROCVector{UInt32}(copy(agent_base_seeds))
...
# ============================================================================
# AMDGPU kernels — apply new infections
# ============================================================================
...
# ============================================================================
# AMDGPU kernels — state transitions (recovery / waning)
# ============================================================================
...
function gpu_step_state!(gsim::GPUSim, disease_name::Symbol; current_ti::Int)
    ...
    if disease isa Starsim.SEIR && gpu_dis.exposed !== nothing
        ...
        @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups _seir_exposure_kernel!(
        ...
        if gpu_dis.recovered !== nothing
            @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups _sir_recovery_kernel!(
    ...
    elseif disease isa Starsim.SIS
        ...
        if gpu_dis.ti_recovered !== nothing
            @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups _sis_recovery_ti_kernel!(
                gpu_dis.infected, gpu_dis.susceptible, gpu_dis.ti_recovered,
                ti_f, n,
            )
        else
            dur_inf_ts = Float32(disease.dur_inf / gsim.sim.pars.dt)
            @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups _sis_recovery_kernel!(
...
function _ensure_edge_capacity!(gsim::GPUSim, n::Int)
    if n > gsim.edge_capacity
        ...
        gsim.edge_p1 = ROCVector{Int32}(zeros(Int32, new_cap))
        gsim.edge_p2 = ROCVector{Int32}(zeros(Int32, new_cap))
        gsim.edge_beta = ROCVector{Float32}(zeros(Float32, new_cap))
        gsim.rng_buf = ROCVector{Float32}(zeros(Float32, new_cap))
...
Uses pre-allocated GPU edge buffers to avoid per-timestep ROCVector
...
function gpu_transmit!(gsim::GPUSim, disease_name::Symbol; current_ti::Int)
    ...
    AMDGPU.fill!(gsim.new_infected, 0x00)
    ...
    for (net_name, net) in gsim.sim.networks
        ...
        groups_e = cld(n_edges, ROCM_THREADS)
        ...
        @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups_e _transmission_kernel!(
    ...
    groups_a = cld(n_agents, ROCM_THREADS)

    if disease isa Starsim.SEIR && gpu_dis.exposed !== nothing
        # SEIR: new infections go to Exposed state (not directly Infected)
        @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups_a _apply_exposures_seir_kernel!(
    ...
    elseif disease isa Starsim.SIR && gpu_dis.ti_recovered !== nothing
        ...
        @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups_a _apply_infections_sir_kernel!(
...
disease, but both execute on the AMD GPU.
...
function gpu_waning!(gsim::GPUSim, disease_name::Symbol; current_ti::Int, wane_dur::Float32)
    ...
    @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups _waning_kernel!(
...
Uses `copyto!` to overwrite existing ROCVector buffers without reallocating.
"""
...
function gpu_reset_rng!(gsim::GPUSim; current_ti::Int)
    ...
    groups = cld(Int(n), ROCM_THREADS)
    @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups _crn_reset_seeds_kernel!(
...
function gpu_step_fused!(gsim::GPUSim, disease_name::Symbol; current_ti::Int)
    ...
    groups_a = cld(Int(n_agents), ROCM_THREADS)
    groups_e = cld(Int(n_edges), ROCM_THREADS)
    ...
    if gsim.crn_mode
        ti_jump = UInt32(current_ti) * CRN_DT_JUMP
        @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups_a _crn_reset_seeds_kernel!(
        ...
        if length(gsim.rng_seeds) < n_edges
            gsim.rng_seeds = ROCVector{UInt32}(rand(UInt32(1):UInt32(0xfffffffe), Int(n_edges)))
        end
    end
    ...
    if disease isa Starsim.SIR && gpu_dis.recovered !== nothing
        ...
        @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups_a _fused_recovery_sir_kernel!(
            gpu_dis.infected, gpu_dis.recovered, gpu_dis.ti_recovered,
            ti_f, n_agents,
        )
        ...
        if beta_dt_f32 > Float32(0)
            ...
            if gsim.crn_mode
                ...
                @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups_e _crn_fused_transmit_sir_kernel!(
                ...
                @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups_e _fused_transmit_sir_kernel!(
                    gsim.new_infected, gsim.snap_infected, gsim.snap_susceptible,
                    gpu_dis.rel_trans, gpu_dis.rel_sus,
                    gsim.edge_p1, gsim.edge_p2, gsim.edge_beta,
    ...
    elseif disease isa Starsim.SIS
        ...
        if beta_dt_f32 > Float32(0)
            ...
            if gsim.crn_mode
                @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups_e _crn_fused_transmit_sis_kernel!(
                    gsim.new_infected, gsim.snap_infected, gsim.snap_susceptible,
                    gpu_dis.rel_trans, gpu_dis.rel_sus,
                    gsim.edge_p1, gsim.edge_p2, gsim.edge_beta,
                    beta_dt_f32, gsim.rng_seeds_agent, gsim.people.slot,
                ...
                @roc groupsize=ROCM_THREADS gridsize=ROCM_THREADS*groups_e _fused_transmit_sis_kernel!(
                    gsim.new_infected, gsim.snap_infected, gsim.snap_susceptible,
                    gpu_dis.rel_trans, gpu_dis.rel_sus,
                    gsim.edge_p1, gsim.edge_p2, gsim.edge_beta,
                    beta_dt_f32, gsim.rng_seeds, n_edges,
...
function cache_edges!(gsim::GPUSim)
    ...
    if length(gsim.rng_buf) < total_edges
        gsim.rng_buf = ROCVector{Float32}(zeros(Float32, total_edges))
    end
...
# Example
```julia
using AMDGPU  # triggers GPU extension loading
