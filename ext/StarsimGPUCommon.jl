...
Shared GPU implementation for Starsim.jl backend extensions.

Backend-specific modules define GPU vector aliases, launch macros, and
synchronization helpers, then include this file to obtain the common
simulation logic.
"""
...
struct GPUPeopleArrays
    alive::GPUVector{UInt8}
    age::GPUVector{Float32}
    female::GPUVector{UInt8}
    slot::GPUVector{Int32}    # CRN slot indices (1-based)
    n::Int
end
...
struct GPUDiseaseArrays
    susceptible::GPUVector{UInt8}
    infected::GPUVector{UInt8}
    recovered::Union{GPUVector{UInt8}, Nothing}
    ti_infected::GPUVector{Float32}
    ti_recovered::Union{GPUVector{Float32}, Nothing}
    rel_sus::GPUVector{Float32}
    rel_trans::GPUVector{Float32}
    n::Int
    # SEIR-specific (nothing for SIR/SIS)
    exposed::Union{GPUVector{UInt8}, Nothing}
    ti_exposed::Union{GPUVector{Float32}, Nothing}
end
...
each timestep via `copyto!` to avoid per-step GPUVector allocation.
"""
mutable struct GPUSim
    ...
    edge_p1::GPUVector{Int32}
    edge_p2::GPUVector{Int32}
    edge_beta::GPUVector{Float32}
    rng_buf::GPUVector{Float32}
    new_infected::GPUVector{UInt8}
    jitter_buf::GPUVector{Float32}
    ...
    crn_mode::Bool
    rng_seeds_base::GPUVector{UInt32}   # per-agent base seeds (deterministic from sim seed)
    rng_seeds_agent::GPUVector{UInt32}  # per-agent timestep seeds (read-only within kernels)
end
...
@inline function _deterministic_seed(base_seed::UInt32, i::Integer)
    s = base_seed ⊻ UInt32((Int(i) * 2654435761) & 0xffffffff)
    return s == UInt32(0) ? UInt32(1) : s
end

function _deterministic_seeds(base_seed::UInt32, n::Int)
    seeds = Vector{UInt32}(undef, n)
    @inbounds for i in 1:n
        seeds[i] = _deterministic_seed(base_seed, i)
    end
    return seeds
end

function _ensure_rng_seed_capacity!(gsim::GPUSim, n::Int)
    length(gsim.rng_seeds) >= n && return

    base_seed = UInt32(gsim.sim.pars.rand_seed & 0xffffffff)
    old_seeds = Array(gsim.rng_seeds)
    new_seeds = Vector{UInt32}(undef, n)
    copyto!(new_seeds, 1, old_seeds, 1, length(old_seeds))
    @inbounds for i in (length(old_seeds) + 1):n
        new_seeds[i] = _deterministic_seed(base_seed, i)
    end
    gsim.rng_seeds = GPUVector{UInt32}(new_seeds)
    return
end
...
function _people_to_gpu(people::Starsim.People)
    ...
    GPUPeopleArrays(
        GPUVector{UInt8}(UInt8.(people.alive.raw)),
        GPUVector{Float32}(Float32.(people.age.raw)),
        GPUVector{UInt8}(UInt8.(people.female.raw)),
        GPUVector{Int32}(Int32.(people.slot.raw)),
        n,
    )
end
...
function _sir_to_gpu(d::Starsim.SIR)
    ...
    GPUDiseaseArrays(
        GPUVector{UInt8}(UInt8.(d.infection.susceptible.raw)),
        GPUVector{UInt8}(UInt8.(d.infection.infected.raw)),
        GPUVector{UInt8}(UInt8.(d.recovered.raw)),
        GPUVector{Float32}(Float32.(d.infection.ti_infected.raw)),
        GPUVector{Float32}(Float32.(d.ti_recovered.raw)),
        GPUVector{Float32}(Float32.(d.infection.rel_sus.raw)),
        GPUVector{Float32}(Float32.(d.infection.rel_trans.raw)),
        n,
...
function _sis_to_gpu(d::Starsim.SIS)
    ...
    GPUDiseaseArrays(
        ...
        GPUVector{Float32}(Float32.(d.infection.ti_infected.raw)),
        GPUVector{Float32}(Float32.(d.ti_recovered.raw)),  # SIS uses ti_recovered for timing
        GPUVector{Float32}(Float32.(d.infection.rel_sus.raw)),
        GPUVector{Float32}(Float32.(d.infection.rel_trans.raw)),
        n,
...
function _seir_to_gpu(d::Starsim.SEIR)
    ...
    GPUDiseaseArrays(
        ...
        GPUVector{UInt8}(UInt8.(d.exposed.raw)),
        GPUVector{Float32}(Float32.(d.ti_exposed.raw)),
    )
end

"""
    to_gpu(sim; backend=:metal/:cuda/:amdgpu) → GPUSim

Copy a CPU simulation's state arrays to GPU memory.
...
"""
function _to_gpu_impl(sim::Starsim.Sim)
    ...
    edge_p1 = GPUVector{Int32}(zeros(Int32, edge_cap))
    edge_p2 = GPUVector{Int32}(zeros(Int32, edge_cap))
    edge_beta = GPUVector{Float32}(zeros(Float32, edge_cap))
    rng_buf = GPUVector{Float32}(zeros(Float32, edge_cap))
    new_infected = gpu_zeros(UInt8, n)
    jitter_buf = GPUVector{Float32}(zeros(Float32, n))
    snap_infected = gpu_zeros(UInt8, n)
    snap_susceptible = gpu_zeros(UInt8, n)
    ...
    edge_seeds = _deterministic_seeds(base_seed, rng_seed_count)
    rng_seeds = GPUVector{UInt32}(edge_seeds)
    ...
    for i in 1:n
        ...
        agent_base_seeds[i] = _deterministic_seed(base_seed, slot_i)
    end
    rng_seeds_base = GPUVector{UInt32}(agent_base_seeds)
    rng_seeds_agent = GPUVector{UInt32}(copy(agent_base_seeds))
...
# ============================================================================
# GPU kernels — apply new infections
# ============================================================================
...
# ============================================================================
# GPU kernels — state transitions (recovery / waning)
# ============================================================================
...
# Pairwise XOR combining: for edge (src, trg), use the two agents'
# timestep-local seeds to produce a single Float32 without mutating shared
# state. This mirrors the CPU CRN structure: one slot-indexed draw per agent
# per timestep, then pairwise XOR combining for the edge draw.
...
# CRN variant of fused SIR transmission: uses per-agent timestep seeds with
# pairwise XOR combining. Reads from snapshot buffers, writes to new_infected.
function _crn_fused_transmit_sir_kernel!(
    ...
    if snap_infected[src] == 0x01 && snap_susceptible[trg] == 0x01
        ...
        rng_val = _crn_pairwise_rng(seed_src, seed_trg)
...
function gpu_step_state!(gsim::GPUSim, disease_name::Symbol; current_ti::Int)
    ...
    if disease isa Starsim.SEIR && gpu_dis.exposed !== nothing
        ...
        cpu_dur = if gsim.crn_mode
            cpu_exposed = Array(gpu_dis.exposed)[1:Int(n)]
            cpu_ti_exposed = Array(gpu_dis.ti_exposed)[1:Int(n)]
            targets = Int[]
            @inbounds for i in 1:Int(n)
                if cpu_exposed[i] == 0x01 && (ti_f - cpu_ti_exposed[i]) >= dur_exp_ts
                    push!(targets, i)
                end
            end
            _sample_recovery_target_buffer_crn(disease, gsim.sim, targets, current_ti, Int(n))
        else
            _sample_recovery_durations_dense(disease, dur_inf_ts, Int(n))
        end
        copyto!(gsim.jitter_buf, 1, cpu_dur, 1, Int(n))

        # Exposed → Infected (latent period elapsed, with recovery time)
        @gpu_launch groups _seir_exposure_kernel!(
        ...
        if gpu_dis.recovered !== nothing
            @gpu_launch groups _sir_recovery_kernel!(
...
function _ensure_edge_capacity!(gsim::GPUSim, n::Int)
    if n > gsim.edge_capacity
        ...
        gsim.edge_p1 = GPUVector{Int32}(zeros(Int32, new_cap))
...
"""
Sample `n` dense recovery durations on CPU matching the disease's
recovery_dist. Used by the non-CRN GPU path.
"""
function _sample_recovery_durations_dense(disease, dur_inf_ts::Float32, n::Int)
...
function _sample_recovery_target_buffer_crn(disease, sim, targets::Vector{Int}, ti::Int, n::Int)
    buf = zeros(Float32, n)
    isempty(targets) && return buf

    draws = Starsim._sample_recovery_draws(disease, sim, Starsim.UIDs(targets), ti)
    @inbounds for (i, u) in pairs(targets)
        buf[u] = Float32(draws[i])
    end
    return buf
end

"""
Build a dense per-agent recovery-duration buffer for the CRN GPU path. Only
agents flagged in `new_infected` receive nonzero durations.
"""
function _sample_recovery_durations_crn(disease, sim, new_infected::AbstractVector{UInt8}, ti::Int, n::Int)
    targets = Int[]
    @inbounds for i in 1:n
        new_infected[i] == 0x01 && push!(targets, i)
    end
    return _sample_recovery_target_buffer_crn(disease, sim, targets, ti, n)
end
...
Uses pre-allocated GPU edge buffers to avoid per-timestep GPUVector
...
function gpu_transmit!(gsim::GPUSim, disease_name::Symbol; current_ti::Int)
    ...
    elseif disease isa Starsim.SIR && gpu_dis.ti_recovered !== nothing
        ...
        cpu_dur = if gsim.crn_mode
            ...
            _sample_recovery_durations_crn(disease, gsim.sim, cpu_new_infected, current_ti, gpu_dis.n)
        else
...
disease, but both execute on the selected GPU backend.
...
function gpu_waning!(gsim::GPUSim, disease_name::Symbol; current_ti::Int, wane_dur::Float32)
    ...
    @gpu_launch groups _waning_kernel!(
...
Uses `copyto!` to overwrite existing GPUVector buffers without reallocating.
"""
...
function gpu_reset_rng!(gsim::GPUSim; current_ti::Int)
    ...
    groups = cld(Int(n), GPU_THREADS)
    @gpu_launch groups _crn_reset_seeds_kernel!(
...
function gpu_step_fused!(gsim::GPUSim, disease_name::Symbol; current_ti::Int)
    ...
    groups_a = cld(Int(n_agents), GPU_THREADS)
    groups_e = cld(Int(n_edges), GPU_THREADS)
    ...
    if gsim.crn_mode
        ...
        _ensure_rng_seed_capacity!(gsim, Int(n_edges))
    end
    ...
    if disease isa Starsim.SIR && gpu_dis.recovered !== nothing
        ...
        @gpu_launch groups_a _fused_recovery_sir_kernel!(
            gpu_dis.infected, gpu_dis.recovered, gpu_dis.ti_recovered,
            ti_f, n_agents,
        )
        ...
        if beta_dt_f32 > Float32(0)
            ...
            cpu_dur = if gsim.crn_mode
                cpu_new_infected = Array(gsim.new_infected)[1:Int(n_agents)]
                _sample_recovery_durations_crn(disease, gsim.sim, cpu_new_infected, current_ti, Int(n_agents))
            else
                _sample_recovery_durations_dense(disease, dur_inf_ts, Int(n_agents))
            end
    ...
    elseif disease isa Starsim.SIS
        ...
        if gpu_dis.ti_recovered !== nothing
            @gpu_launch groups_a _sis_recovery_ti_kernel!(
                gpu_dis.infected, gpu_dis.susceptible, gpu_dis.ti_recovered,
                ti_f, n_agents,
            )
        else
            @gpu_launch groups_a _sis_recovery_kernel!(
                gpu_dis.infected, gpu_dis.susceptible, gpu_dis.ti_infected,
...
# ============================================================================
# High-level run_gpu! implementation shared by all GPU backends
# ============================================================================
...
"""
    Starsim.run_gpu!(sim::Sim; verbose::Int=1, backend=:metal/:cuda/:amdgpu, cache_edges::Bool=false)

GPU backend implementation for `run!`. Called via backend-specific dispatch.
...
function _run_gpu_impl!(sim::Starsim.Sim; verbose::Int=1, cache_edges::Bool=false)
    ...
    if verbose >= 1
        println("Running simulation on GPU ($(n_agents) agents, $(npts) steps, " *
                "dt=$(sim.pars.dt), device=$(gpu_device()))")
    end

    gsim = _to_gpu_impl(sim)
