...
"""
function Starsim._to_gpu_backend(sim::Starsim.Sim, ::Val{:metal})
...
function gpu_step_fused!(gsim::GPUSim, disease_name::Symbol; current_ti::Int)
    ...
    elseif disease isa Starsim.SIS
        ...
        if gpu_dis.ti_recovered !== nothing
            @metal threads=METAL_THREADS groups=groups_a _sis_recovery_ti_kernel!(
                gpu_dis.infected, gpu_dis.susceptible, gpu_dis.ti_recovered,
                ti_f, n_agents,
            )
        else
...
```
"""
function Starsim._run_gpu_backend!(sim::Starsim.Sim, ::Val{:metal}; verbose::Int=1, cache_edges::Bool=false)
    if !sim.initialized
        Starsim.init!(sim)
    end
    ...
    gsim = Starsim._to_gpu_backend(sim, Val(:metal))
