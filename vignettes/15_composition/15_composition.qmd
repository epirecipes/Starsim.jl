---
title: "Category-Theory Composition with Catlab.jl"
author: "Simon Frost"
format:
  html:
    code-fold: false
...
One challenge in agent-based disease modeling is composing multiple modules
(diseases, networks, demographics) into a coherent simulation without
introducing subtle bugs. Starsim.jl provides a **category-theory-based
composition** framework via the `Catlab.jl` extension, allowing you to
formally specify how modules share state and then automatically assemble
a simulation.
...
transition structure of a disease model. Each state (S, I, R, etc.) is an
object, and each transition (infection, recovery) is a morphism from source
to target state.
...
This is equivalent to the textbook SIR compartmental diagram:
**S → I → R**, where infection moves agents from susceptible to infected,
and recovery moves them from infected to recovered.
...
which agent states it shares as "ports". When two sharers declare the same
port (e.g., `:alive`), composing them ensures they operate on the same
population.

```{julia}
# Create modules
sir = SIR(beta=0.05, dur_inf=4.0, init_prev=0.01)
...
disease_sharer_2 = EpiSharer(:sir_disease_2, SIR(beta=0.05, dur_inf=4.0))
network_sharer_2 = EpiSharer(:contact_net_2, RandomNet(n_contacts=10))
...
`compose_epi` takes a vector of sharers and assembles them into a complete
simulation:
...
The composed and manual simulations are **identical** — same disease dynamics,
same random number streams, same results. This is the functoriality guarantee:
...
sir2 = SIR(beta=0.05, dur_inf=4.0, init_prev=0.01)
net2 = RandomNet(n_contacts=10)
...
For hierarchical composition, `OpenEpiNet` creates structured cospans that
expose specific disease states as ports for gluing:
...
This enables compositional design where disease models can be connected at
shared states — for example, two diseases sharing a susceptible population
through a connector.
...
1. **Formal correctness**: Shared states are identified at composition time —
   no silent mismatches or forgotten connections.
2. **Modularity**: Each component is self-contained with explicit interfaces.
3. **Compositionality**: Complex models are built by composing simpler ones,
   with guarantees that the composition preserves each component's behavior.
...
[AlgebraicJulia](https://www.algebraicjulia.org/) and the applied category
theory approach to scientific modeling.
