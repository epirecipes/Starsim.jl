...
function Starsim.init_pre!(d::HPVGenotype, sim)
    ...
    all_states = [
        ...
        d.will_cin, d.will_cancer, d.date_cin, d.date_cancer,
...
"""
function _set_prognosis!(d::HPVGenotype, uid::Int, ti::Int)
    gp = d.params
    rng = d.rng
    dt = 0.25  # default dt, will be adjusted if available

    # Sample pre-CIN duration (years)
    ...
    if rand(rng) < cin_prob
        ...
        # Schedule CIN date (current ti + dur_precin in timesteps)
        d.date_cin.raw[uid] = Float64(ti) + dp / dt

        # Sample CIN duration (years)
        ...
        if rand(rng) < cancer_prob
            ...
            d.date_cancer.raw[uid] = d.date_cin.raw[uid] + dc / dt
        end
    end
    return
end
...
function _step_duration_model!(d::HPVGenotype, sim, active::Vector{Int}, ti::Int, dt::Float64)
    ...
    @inbounds for u in active
        ...
        # Check clearance first (probabilistic per-timestep, same as rate model)
        if !d.cin1.raw[u] && !d.cin2.raw[u] && !d.cin3.raw[u]
            # Pre-CIN clearance
            p_clear = 1.0 - (1.0 - gp.clearance_rate_inf)^dt
            if rand(d.rng) < p_clear
        ...
        # Check scheduled CIN development
        if d.will_cin.raw[u] && !d.cin1.raw[u] && !d.cin2.raw[u] && !d.cin3.raw[u]
            if ti_f >= d.date_cin.raw[u] && sim.people.female.raw[u]
                d.cin1.raw[u] = true
                d.ti_cin1.raw[u] = ti_f
            end
        end

        # CIN clearance and staging (severity-based progression through CIN stages)
        if d.cin1.raw[u] && !d.cin2.raw[u]
            p_clear_cin1 = 1.0 - (1.0 - gp.clearance_rate_cin1)^dt
            if rand(d.rng) < p_clear_cin1
                d.cin1.raw[u] = false
                _clear_infection!(d, u, ti_f)
                _notify_clearance(d, sim, u)
                continue
            end
            # Severity-based progression: CIN1 → CIN2
        ...
        if d.cin2.raw[u] && !d.cin3.raw[u]
            p_clear_cin2 = 1.0 - (1.0 - gp.clearance_rate_cin2)^dt
            if rand(d.rng) < p_clear_cin2
                d.cin2.raw[u] = false
                _clear_infection!(d, u, ti_f)
                _notify_clearance(d, sim, u)
                continue
        ...
        if d.cin3.raw[u]
            p_clear_cin3 = 1.0 - (1.0 - gp.clearance_rate_cin3)^dt
            if rand(d.rng) < p_clear_cin3
                d.cin3.raw[u] = false
                _clear_infection!(d, u, ti_f)
                _notify_clearance(d, sim, u)
                continue
            end
            # Check scheduled cancer
...
function _infect_hpv!(d::HPVGenotype, sim)
    ...
    for (net_name, net) in sim.networks
        ...
        new_infections += _infect_hpv_standard!(d, sim, edges, beta_dt, ti, net)
    end

    return new_infections
end

function _infect_hpv_standard!(d::HPVGenotype, sim, edges::Starsim.Edges, beta_dt::Float64, ti::Int, net)
    new_infections = 0
    ...
    @inbounds for i in 1:n_edges
        ...
        # src → trg transmission
        if infected_raw[src] && susceptible_raw[trg]
            trans_mult = female_raw[trg] ? M2F_TRANS_RATIO : 1.0
            p = rel_trans_raw[src] * rel_sus_raw[trg] * beta_dt * edge_beta * trans_mult
            if rand(rng) < p
        ...
        # Bidirectional: trg → src
        if bidir && infected_raw[trg] && susceptible_raw[src]
            trans_mult = female_raw[src] ? M2F_TRANS_RATIO : 1.0
            p = rel_trans_raw[trg] * rel_sus_raw[src] * beta_dt * edge_beta * trans_mult
            if rand(rng) < p
...
function _do_hpv_infection!(d::HPVGenotype, sim, target::Int, source::Int, ti::Int)
    ...
    # Set prognosis for duration model
    if d.use_duration_model
        _set_prognosis!(d, target, ti)
    end
