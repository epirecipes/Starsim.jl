...
function main()
    ...
    if n_pass == n_total
        ...
        println()
        println("Key implementation differences (expected causes of non-overlap):")
        println("  1. Network architecture: Python StructuredSexual has complex risk-group")
        println("     stratification (f0=0.85/f1=0.1/f2=0.05), sex work, age-bin mixing,")
        println("     and p_pair_form=0.5 per woman per timestep. Julia uses simplified")
        println("     [0.5/0.3/0.2] groups with age-matching and turnover-based formation.")
        println("     → Causes Julia to spread slower (lower yr5/yr10) but more sustained.")
        println("  2. HIV mortality: Python applies CD4-bin mortality continuously + AIDS")
        println("     death at ti_zero. Julia has both but epidemic shape differs → slower")
        println("     decline in Julia (higher yr30/yr40).")
        println("  3. Duration distributions: Julia Normal(μ,0.3μ) vs Python LogNormal(μ,σ)")
        println()
        println("Metrics expected to agree (same effective R0/steady-state):")
        println("  ✓ HIV prev @ yr20 — epidemic crossing point")
        println("  ✓ Syphilis prev @ yr20 — epidemic crossing point")
        println("  ✓ N tertiary @ yr20 — matched time_to_tertiary distribution")
    end
    println("=" ^ 78)
